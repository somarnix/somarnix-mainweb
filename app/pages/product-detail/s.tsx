...
export default function ProductDetailPage({
  ...
  /* ================= DERIVED (HOOKS MUST BE HERE, BEFORE RETURNS) ================= */
  ...
  const mainPrice = useMemo(() => {
    return selectedVariant?.price ?? product?.min_price ?? null;
  }, [selectedVariant, product]);

  const mainOriginal = useMemo(() => {
    return selectedVariant?.original_price ?? product?.min_original_price ?? null;
  }, [selectedVariant, product]);
  ...
  const modalVariants = useMemo(() => {
    return variants
      ...
      .map((v) => ({
        ...
        label:
          v.duration_label ??
          (v.duration_days ? `${v.duration_days} Days` : "Option"),
        price: v.price,
      }));
  }, [variants]);
  ...
  /* ================= SAFE NORMAL VALUES (AFTER PRODUCT EXISTS) ================= */
  ...
  // ✅ No filter(Boolean) => no TS error
  const featureList: string[] = [];
  if (product.level) featureList.push(`Level: ${product.level}`);
  featureList.push(
  ...
  // ✅ Buy now => add to cart immediately (qty=1)
  const handleBuyNow = async () => {
    const variantId =
      selectedVariantId ??
      variants.find((v) => typeof v.price === "number")?.id ??
      null;
    ...
    try {
      ...
      if (!res.ok) {
        const msg = await res.text().catch(() => "");
        throw new Error(msg || "ADD_CART_FAILED");
      }

      // Optional: if you have cart page, redirect:
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 py-8">
        <div className="flex flex-col lg:flex-row gap-8 items-start">
          ...
          {/* RIGHT */}
          <aside className="w-full lg:w-[380px] xl:w-[420px] lg:shrink-0">
            <div className="mb-5">
                          ...
                          <div className="flex items-center justify-between gap-3">
                            ...
                            <div className="text-right">
                              <div className="font-bold text-gray-900 dark:text-white">
                                {typeof v.price === "number" ? formatPrice(v.price) : "-"}
                              </div>
        ...
        <div className="mt-10">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2">
            {related.map((p) => (
              <button
                ...
                className="
                  border rounded-xl overflow-hidden bg-white dark:bg-gray-900
                  shrink-0
                  min-w-[33.33%] sm:min-w-[25%] lg:min-w-[20%]
                "
              >
                <img
          ...
          {relatedLoading && (
            <div className="mt-3 text-sm text-gray-500">Loading related...</div>
          )}
        </div>
      </div>

      {/* OPTIONAL MODAL (keep if you still want it elsewhere) */}
      {showCartModal && (
        <AddToCartModal
