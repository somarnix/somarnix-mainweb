...
function safeCopyRecursive(sourcePath, targetPath) {
  ...
  if (sourceStat.isDirectory()) {
    ...
    for (const entry of entries) {
      ...
      safeCopyRecursive(
        joinFsPath(sourcePath, entry.name),
        joinFsPath(targetPath, entry.name)
      );
    }
    return;
  }
...
function prepareAutomationUserDataDir(sourceUserDataDir, profileDirectory) {
  const automationRoot =
    ...
    joinFsPath(process.cwd(), ".flow-chrome-runtime");
  const targetUserDataDir = joinFsPath(
    automationRoot,
    sanitizePathSegment(profileDirectory)
  );
  ...
  if (sourceUserDataDir) {
    safeCopyRecursive(
      joinFsPath(sourceUserDataDir, "Local State"),
      joinFsPath(targetUserDataDir, "Local State")
    );
    safeCopyRecursive(
      joinFsPath(sourceUserDataDir, profileDirectory),
      joinFsPath(targetUserDataDir, profileDirectory)
    );
  }
...
async function loadAutomationLibrary() {
  ...
  try {
    const puppeteer = await importOptionalModule("puppeteer");
  ...
  try {
    const puppeteerCore = await importOptionalModule("puppeteer-core");
...
function detectUserDataDir() {
  ...
  const candidate = joinFsPath(localAppData, "Google", "Chrome", "User Data");
...
function resolveProfileDirectory(userDataDir, requestedProfile) {
  ...
  const localStatePath = joinFsPath(userDataDir, "Local State");
