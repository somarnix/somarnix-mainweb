      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => onNavigate("orders")}>
              ...
              {lang === "km" ? "ត្រឡប់ទៅការបញ្ជាទិញ" : "Back to orders"}
            </Button>
        ...
        <div
          className={`grid grid-cols-1 gap-6 ${
            selectedConversation ? "lg:grid-cols-[320px,1fr]" : ""
          }`}
        >
            ...
            <div className="flex-1 overflow-y-auto space-y-2">
              {conversationsLoading ? (
                ...
                filteredConversations.map((conv) => {
                  ...
                  const party = isAdmin ? conv.buyer : conv.seller;
                  const displayName =
                    party?.name ||
                    party?.email ||
                    (isAdmin
                  ...
                  const avatarLetter = (displayName || "")
                    ...
                    .slice(0, 2)
                    .toUpperCase();
                  const online = party?.presence?.status === "online";
                ...
                <div className="flex items-center gap-3">
                  {(() => {
                    ...
                    const label = isAdmin
                      ...
                      ? "អ្នកលក់"
                      : "Seller";
                    const name = party?.name || party?.email || label;
                    const initials = name.trim().slice(0, 2).toUpperCase();
                    const online =
                      party?.presence?.status === "online" ||
                      (!!party?.presence?.lastActiveAt &&
                        Date.now() -
                          new Date(party.presence.lastActiveAt).getTime() <=
                          5 * 60 * 1000);
                    return (
                      <div className="relative">
                        {party?.avatarUrl ? (
